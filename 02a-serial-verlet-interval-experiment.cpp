...
#define MAX_INTERACTION_RADIUS 20
...
int main(int argc, char* argv[]) {
  ...
  const char* outputPrefix = argv[argc-2];
  const int verlet_interval = atoi(argv[12]);
    ...
    if((s - 1) % verlet_interval == 0) {
      for (int i = 0; i < n; i++) {
        ...
        for (int j = i+1; j < n; j++) {
          ...
          if(dist <= MAX_INTERACTION_RADIUS + RADIUS_BUFFER_PER_STEP * (verlet_interval - 1)) {
